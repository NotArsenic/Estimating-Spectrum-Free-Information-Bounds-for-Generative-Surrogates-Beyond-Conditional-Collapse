"""
CLI entry point for MDN training and evaluation.
...
from model import Multivariate_MDN
...
from plots_mdn import plot_mdn_internals
...
def generate_dump(
    ...
    def sample_fn(Xb, m):
        with torch.no_grad():
            ...
            return model.sample(xb, n_samples=m).cpu().numpy()
    ...
    def internals_fn(Xs):
        ...
        with torch.no_grad():
            for s in range(0, len(Xs), batch_size):
                e = min(s + batch_size, len(Xs))
                pi, mu, L = model(torch.from_numpy(Xs[s:e]).float().to(device))
                pis.append(pi.cpu().numpy().astype(np.float32))
                mus.append(mu.cpu().numpy().astype(np.float32))
                Ls.append(L.cpu().numpy().astype(np.float32))
        return {
...
def evaluate_model(
    ...
    print(f"\n{'='*60}")
    print(f"  Evaluating MDN V2 checkpoint  (stage={stage})")
    print(f"{'='*60}")
    ...
    # 1. Exact log-likelihood (closed form!)
    print("[eval] Computing exact log-likelihood...")
    ll_values = []
    for start in range(0, N_test, batch_size):
        end = min(start + batch_size, N_test)
        X_batch = torch.from_numpy(X_test[start:end]).float().to(device)
        y_batch = torch.from_numpy(y_test[start:end]).float().to(device)
        ll = compute_log_likelihood(model, X_batch, y_batch)
        ll_values.append(ll.cpu().numpy())

    ll_all = np.concatenate(ll_values, axis=0)
    mean_ll = float(np.mean(ll_all))
    print(f"[eval] Mean log-likelihood: {mean_ll:.4f}")

    # 2. Generate single samples
    ...
    for start in range(0, N_test, batch_size):
        ...
        X_batch = torch.from_numpy(X_test[start:end]).float().to(device)
        s = model.sample(X_batch, n_samples=1)  # (B, D)
        all_samples.append(s.cpu().numpy())

    sampled = np.concatenate(all_samples, axis=0)

    # 3. Generate multiple samples (for CRPS / coverage)
    ...
    for start in range(0, n_multi, batch_size):
        ...
        X_batch = torch.from_numpy(X_test[start:end]).float().to(device)
        s = model.sample(X_batch, n_samples=n_eval_samples)  # (B, M, D)
        multi_samples.append(s.cpu().numpy())

    sampled_multi = np.concatenate(multi_samples, axis=0)  # (n_multi, M, D)

    # 4. PIT values (exact mixture CDF — no sampling noise)
    print("[eval] Computing exact PIT values...")
    pit_values = []
    n_pit = min(N_test, 100_000)
    for start in range(0, n_pit, batch_size):
        end = min(start + batch_size, n_pit)
        X_batch = torch.from_numpy(X_test[start:end]).float().to(device)
        y_batch = torch.from_numpy(y_test[start:end]).float().to(device)
        pit = compute_pit(model, X_batch, y_batch)
        pit_values.append(pit.cpu().numpy())

    pit_all = np.concatenate(pit_values, axis=0)

    # 4b. Joint 4-D coverage via exact HDR rank
    print("[eval] Computing joint HDR-rank coverage...")
    seed_torch(seed + EVAL_HDR)
    n_hdr = min(N_test, 100_000)
    lp_truth, lp_samp = [], []
    for start in range(0, n_hdr, batch_size):
        end = min(start + batch_size, n_hdr)
        X_batch = torch.from_numpy(X_test[start:end]).float().to(device)
        y_batch = torch.from_numpy(y_test[start:end]).float().to(device)
        t, s = compute_hdr_logp(model, X_batch, y_batch, n_samples=n_eval_samples)
    ...
    #    5. Collect source (this stage's conditioner) kinematics
    ...
    metrics["n_ll"] = N_test  # closed-form: likelihood computed on the full test split

    metrics["config_hash"] = stats.get("config_hash")

    metrics.update({f"exact_{k}": v for k, v in compute_pit_ks(pit_all).items()})
    ...
    for _bs in (256, 1024, 4096):
        ...
        _Xs = torch.from_numpy(X_test[:_bs_eff]).float().to(device)
        with torch.no_grad():
            for _ in range(_n_warmup):
                _ = model.sample(_Xs, n_samples=1)
        ...
        speed_results[f"MDN bs={_bs_eff}"] = (_t1 - _t0) * 1e6 / (_n_iters * _bs_eff)
    ...
    # dijet_order is sorted by group; consecutive pairs are the two jets
    ...
    # MDN-specific figures (mixture internals, Cholesky)
    plot_mdn_internals(model, X_test, device, fig_dir)
    ...
    for name in TARGET_NAMES:
        ...
        ks = metrics.get(f"exact_pit_ks_{name}", float("nan"))
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="MDN Training & Evaluation",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        "Output/mdn/ for parton2reco, "
        "Output/mdn/Experiments/<stage>/ otherwise. "
        "A bare name (e.g. 'ablation') is placed under "
        "Output/mdn/Experiments/.",
    )
    parser.add_argument(
        "--run_tag",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=8,
        help="Mixture components. Default 8, not 32: V1 measured a K=32 "
        "mixture collapsing to ~4 effective components, with pruning to "
        "the top-5 exactly lossless — the conditional density is "
        "near-unimodal. K=8 has less margin against early component "
        "death than K=32, so if pi collapses before components have "
        "learned, that is a different failure from benign collapse.",
    )

    # Regularisation
    parser.add_argument("--pi_tau_start", type=float, default=1.5)
    parser.add_argument("--pi_tau_anneal_frac", type=float, default=0.5)
    parser.add_argument(
        "--lambda_calib",
        type=float,
        default=0.1,
        help="Weight on the pi-weighted mixture calibration penalty "
        "(E[D^2] -> D). Default 0.1 matches MDN_V1. Pass 0 for the "
        "ablation arm; model selection is on pure val NLL either way.",
    )
    ...
    parser.add_argument(
        ...
        "Instrumentation only -- model selection stays on val NLL.",
    )
    ...
    if args.save_root:
        save_root = str(
            _paths.model_output_root("mdn") / "Experiments" / Path(args.save_root).name
        )
    else:
        save_root = str(_paths.save_root("mdn", args.stage))
    ...
    for seed in args.seeds:
        ...
        if args.dump_only:
            generate_dump(
                ...
                n_hdr_samples=args.n_eval_samples,
        ...
        best_val_nll = train(
            ...
            pi_tau_start=args.pi_tau_start,
            pi_tau_anneal_frac=args.pi_tau_anneal_frac,
